...
impl Worker {
    ...
    pub fn do_login(&mut self, login_params: LoginParams) -> Result<(), String> {
        ...
        self.error = true;
        return Err("Login Failed to get your ID, please visit https://mwcpool.com and create an account".to_string());
    }
    ...
    pub fn get_worker_stats(&mut self, login_params: LoginParams) -> Result<(), String> {
        ...
        let mut response = client
            .get(format!("http://poolapi:{}/worker/stats/{}/0,1", self.config.mwc_node.api_port, self.user_id).as_str())
