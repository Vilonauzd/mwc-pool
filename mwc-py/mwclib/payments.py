...
# Call flow for mwc API V2 is the following:
# File send:
#     Sender: init_send_tx
...
def validateAddress(address, adderss_type, logger):
    logger.warning("validateAddress with {} {}".format(address, adderss_type))
    if adderss_type in ["http", "https"]:
        try:
            logger.warning("Validating http wallet address: {}".format(address))
            return urlparse(address).scheme in ['http', 'https', 'mwcmqs']
...
def testWalletPort(address, logger):
    try:
        logger.warning("testWalletPort: {}".format(address))
        s = socket.socket()
        s.settimeout(2)
        ...
        if ":" in parsed.netloc:
            ...
            port = int(port)
        logger.warning("Testing: {}, {}".format(addr, port))
        s.connect((addr, port))
        s.close()
    except Exception as e:
...
def get_tx_slate(user_id, logger, database, method, invoked_by):
    ...
    try:
        ...
        if locked_utxo is None or locked_utxo.amount < (0.2 * 1000000000):
        ...
        try:
            ...
            logger.warning("Requesting Payment slate from payment request api: {}".format(args))
...
def cancel_tx_slate(tx_slate_id, new_state, logger, database):
    try:
        logger.warning("In cancel_tx_slate")
        ...
        if len(wallet_rec) == 0:
            logger.warning("Wallet has no record of tx_slate_id: {}".format(tx_slate_id))
        else:
            logger.warning("XXX: wallet_rec = {}".format(wallet_rec))
            ...
            if wallet_rec[0]["tx_type"] == "TxSentCancelled":
                logger.warning("Tx already marked canceled: {}".format(tx_slate_id))
            else:
...
def submit_tx_slate(user_id, slate, logger, database):
    if slate is None:
        ...
        raise PaymentError(400, message)
    try:
        slate_json = json.loads(slate.decode('utf-8'))['slate']
        tx_id = slate_json["id"]
    except Exception as e:
    ...
    try:
        logger.warning("Running submit_slate: tx_id = {}".format(tx_id))
        # Record Keeping
        timestamp = datetime.utcnow()
        locked_utxo = Pool_utxo.get_locked_by_userid(user_id)
...
def http_send(method, user_id, address, amount, logger, num_change_outputs=1):
    ...
    args = {
            ...
            'selection_strategy_is_use_all': False,
            'message': "pool payment: {}: user_id={}".format(method, user_id),
            'target_slate_version': None,
            'send_args': send_args,
        }
    try:
        ...
        if "is recipient listening" in str(e):
            ...
            raise PaymentError(500, str(e))
    logger.warning("Sent OK: user_id={} - address={} - amount {}".format(user_id, address, amount))
...
def atomic_send(user_id, address, logger, database, method, invoked_by):
    ...
    if valid == False:
        ...
        logger.warning(message)
        raise PaymentError(400, message)
    if (method == "http" or method == "https") and '.onion' not in address:
    ...
    try:
        timestamp = datetime.utcnow()
        # Send the TX
        if method in ("http", "mwcmqs"):
            slate = http_send(method, user_id, address, amount, logger)
    ...
    # Post the TX    
    try:
        logger.warning("Debug: Post the TX")
