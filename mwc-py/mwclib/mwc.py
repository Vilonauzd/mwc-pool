...
#
# Routines for getting mwc Network data
#
...
def get_api_auth():
    ...
    auth = (config["mwc_node"]["api_user"], config["mwc_node"]["api_secret"])
    return auth

def get_api_url():
    config = lib.get_config()
    mwc_api_url = "http://" + config["mwc_node"]["address"] + ":" + config["mwc_node"]["api_port"]
    return mwc_api_url

def get_owner_api_url():
    config = lib.get_config()
    mwc_owner_api_url = "http://" + config["wallet"]["owner_api_address"] + ":" + config["wallet"]["owner_api_port"] + "/v2/owner"
    return mwc_owner_api_url

# Network chain height
def get_current_height():
    config = lib.get_config()
    auth = get_api_auth()
    mwc_api_url = get_api_url()
...
def get_block_by_height(height):
    ...
    auth = get_api_auth()
    mwc_api_url = get_api_url()
    blocks_url = mwc_api_url + "/v1/blocks/" + str(height)
    try:
...
# https://github.com/mimblewimble/mwc/blob/6980278b95d266f6a420b64052ab6231a6e1c466/core/src/consensus.rs#L157
def graph_weight(edge_bits):
    return (2 << (edge_bits - BASE_EDGE_BITS)) * edge_bits

# The following is from
# https://github.com/mimblewimble/mwc-explorer/blob/01b5fc1ebecdb7ec842be241b725675497419ccc/mwcexplorer/blockchain/models.py
