...
# Verify our mysql chain against the mwc network
#  Add missing blocks.
#  Mark orphan blocks.
...
def main():
    ...
    latest = mwc.get_current_height() - 10  # stop 10 blocks from current to avoid overrunning the blockWatcher
    ...
    LOGGER.warning("Starting from block #{}".format(last))

    for i in range(last, latest+1):
        if i % 100 == 0:
            LOGGER.warning("Processing #{}".format(i))
        response = mwc.blocking_get_block_by_height(i)
        ...
        try:
            ...
            if rec is not None:
                ...
                if rec.hash != response["header"]["hash"] and rec.state != "orphan":
                    LOGGER.warning("Found an orphan - height: {}, hash: {} vs {}".format(rec.height, rec.hash, response["header"]["hash"]))
                    rec.state = "orphan"
                    database.db.getSession().commit()
            else:
                # If it was not in the DB then we should add it now
                LOGGER.warning("Adding missing block - height: {}".format(response["header"]["height"]))
