...
#  Request chain height from mwc core every x seconds.
#  If the height increased request each block from mwc core.
...
def main():
    ...
    while current_height <= 0:
        LOGGER.warning("Waiting for first block height")
        sleep(10)
        current_height = mwc.blocking_get_current_height()
    LOGGER.warning("current_height = {}".format(current_height))
    ...
    LOGGER.warning("last_height = {}".format(last_height))
    ...
    while True:
        try:
            ...
            LOGGER.warning("latest: {}, height: {}".format(latest, height))
            while latest >= height:
                response = mwc.blocking_get_block_by_height(height)
                LOGGER.warning("New Block: {} at {}".format(response["header"]["hash"],
                ...
                try:
                    ...
                    LOGGER.warning("Attempted to re-add block: {}".format(response["header"]["height"]))
