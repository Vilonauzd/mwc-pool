...
from mwcbase.model.pool_credits import Pool_credits

# NOTE:  All calculations are in nanomwc
...
def main():
    ...
    chain_height = mwc.blocking_get_current_height()
    ...
    LOGGER.warning("Paying for {} pool blocks: {}".format(len(unlocked_blocks), unlocked_blocks))
    for height in unlocked_blocks:
        try:
            LOGGER.warning("Processing unlocked block: {}".format(height))
            ...
            for user_id, payment_amount in payout_map.items():
                    ...
                    LOGGER.warning("Credit to user: {} = {}".format(user_id, payment_amount))
