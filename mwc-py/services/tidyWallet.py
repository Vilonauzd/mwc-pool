...
def cancel_expired_tx(database, mwc_owner_api_url, tx_timeout):
    global LOGGER
    global CONFIG
    LOGGER.warning("Running cancel_expired_tx ---")
    ...
    for tx in sent_txs:
        ...
        if (datetime.utcnow() - tx.timestamp) > tx_timeout:
            ...
            try:
                ...
                LOGGER.warning("Failed to cancel expired transaction: {}".format(repr(e)))
...
def confirm_tx(database, mwc_owner_api_url):
    global LOGGER
    global CONFIG
    LOGGER.warning("Running confirm_tx ---")
    ...
    for payment_rec in posted_txs:
        ...
        if payment_rec.tx_data is None:
            LOGGER.warning("Skipping confirmation check on payment record {} - no tx data".format(payment_rec.id))
            continue
        try:
            tx_slate = json.loads(payment_rec.tx_data)
            tx_id = tx_slate["id"]
        except Exception as e:
            LOGGER.warning("Skipping confirmation check on payment record {} - invalid tx_data: {}".format(payment_rec.id, str(e)))
            continue
        try:
            txs = wallet.retrieve_txs(tx_slate_id=tx_id)
        except Exception as e:
            LOGGER.warning("Skipping confirmation check on payment record {} - Failed to retrieve from wallet: {}".format(payment_rec.id, str(e)))
            continue
        ...
        try:
            ...
            for tx in txs:
                if tx["tx_type"] == "TxSent" and tx["confirmed"] == True:
                    LOGGER.warning("Confirmed tx: {}".format(tx_id))
                    payment_rec.state = "confirmed"
                    database.db.getSession().commit()
        except Exception as e:
            LOGGER.warning("Failed to confirm payment record {}: tx {}: {}".format(payment_rec.id, tx_id, str(e)))
            continue
...
# Re-post a posted tx
def repost_tx(database, mwc_owner_api_url, tx_timeout_delta):
    global LOGGER
    global CONFIG
    LOGGER.warning("Running repost_tx ---")

    pending_txs = Pool_payment.get_by_state_and_agelimit("posted", tx_timeout_delta)
    for payment_rec in pending_txs:
        LOGGER.warning("processing payment_rec: {}".format(payment_rec))
        if payment_rec.tx_data is None:
            LOGGER.warning("Skipping repost check on payment record {} - no tx data".format(payment_rec.id))
            continue
        try:
            tx_slate = json.loads(payment_rec.tx_data)
            tx_id = tx_slate["id"]
        except Exception as e:
            LOGGER.warning("Skipping repost on payment record {} - invalid tx_data: {}".format(payment_rec.id, str(e)))
            continue
        try:
            txs = wallet.retrieve_txs(tx_slate_id=tx_id)
        except Exception as e:
            LOGGER.warning("Skipping repost on payment record {} - Failed to retrieve from wallet: {}".format(payment_rec.id, str(e)))
            continue
        ...
        try:
            ...
            for tx in txs:
                ...
                if tx["tx_type"] == "TxSent" and tx["confirmed"] == False:
                    LOGGER.warning("Re-posting tx: {}".format(tx_id))
                    res = wallet.post_tx(tx_slate["tx"])
                    print("XXX repost result = {}".format(res))
        except Exception as e:
            LOGGER.warning("Failed to re-post payment record {}: tx {}: {}".format(payment_rec.id, tx_id, str(e)))
            continue
...
def main():
    ...
    database = lib.get_db()
    mwc_owner_api_url = mwc.get_owner_api_url()
    ...
    try:
        repost_tx(database, mwc_owner_api_url, tx_repost_delta)
    except Exception as e:
