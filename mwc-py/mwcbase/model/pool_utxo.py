...
class Pool_utxo(Base):
    ...
    min_payout = Column(BigInteger)
    enable_auto_payout = Column(Boolean)
    ...
    def to_json(self, fields=None):
        obj = {
                ...
                'enable_auto_payout': self.enable_auto_payout
        }
        # Filter by field(s)
        if fields != None:
    ...
    @classmethod
    def update_min_payout(cls, user_id, value):
        try:
            worker_utxo = Pool_utxo.get_locked_by_userid(int(user_id))
            if worker_utxo is None:
                return False
    ...
    @classmethod
    def update_enable_auto_payout(cls, user_id, value):
        try:
            worker_utxo = Pool_utxo.get_locked_by_userid(int(user_id))
            if worker_utxo is None:
                return False
