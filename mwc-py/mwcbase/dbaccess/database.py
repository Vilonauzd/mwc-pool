...
import mwcbase
from mwcbase.model import initialize_sql
...
class database_details:
    def __init__(self, MYSQL_CONSTANTS, user=None):
        ...
        # Create database if not exists
            if MYSQL_CONSTANTS.mysql_db not in existing_databases:
                connection.execute(text("CREATE DATABASE {0}".format(MYSQL_CONSTANTS.mysql_db)))
    ...
    def initialize(self):
        ...
        self.session[threading.get_ident()] = scoped_session(mwcbase.model.session_factory)
