...
export const fetchRigData = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    for (const blockHeight in newRigData) { // for each block
      const blockShareData = { height: parseInt(blockHeight), 'Total': { c31ValidShares: 0 } }
      ...
      for (const rig in newRigData[blockHeight]) {
        for (const worker in newRigData[blockHeight][rig]) {
          ...
          blockShareData[`${rig}-${worker}`] = { c31ValidShares: 0 }
    ...
    /*
    * minerShareData
    * { block_123123: { height: 123123, c31ValidShares: 85 }}
    */
...
export const fetchMinerPaymentTxSlate = () => async (dispatch: Dispatch, getState: GetState) => {
  ...
  try {
    ...
    if (!minerPaymentTxSlateResponse.ok) {
      const errorMessageObject = await minerPaymentTxSlateResponse.json()
      dispatch({
...
export const finalizeTxSlate = (slate) => async (dispatch: Dispatch, getState: GetState) => {
  dispatch({
  ...
  try {
    ...
    const data = {
      ...
      body: JSON.stringify({ slate }) // Pass the slate data in the request body
    }
...
export const setPaymentMethodSetting = (formState: any) => async (dispatch: Dispatch, getState: GetState) => {
  ...
  try {
    ...
    if (formState.paymentType === 'manual') {
      ...
      let url
      if (cleanedRecipient) {
        url = `${API_URL}pool/payment/${method}/${id}/${cleanedRecipient}`
      } else {
        url = `${API_URL}pool/payment/${method}/${id}`
      }
      ...
      // const enablePayoutURL = `${API_URL}worker/utxo/${id}/address/${formState.enablePayout}`
      // const enablePayoutSetting = await fetch(addressUrl, authorizedPost)
      // if (!setPaymentSettingResponse.ok) return
...
export const fetchMinerLatestBlockReward = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    const state = getState()
    const id = state.auth.account.id
    const recentPoolBlocks = state.mwcPoolData.recentBlocks.map(block => block.height)
    ...
    dispatch({
      type: 'MINER_LATEST_BLOCK_MWC_EARNED',
...
export const fetchMinerNextBlockReward = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    dispatch({
      type: 'MINER_NEXT_BLOCK_MWC_EARNED',
