...
import type { NetworkBlockData, MWCPoolBlockData, MinerShareData } from '../../types'
...
export type MinerBlockRewardsStateProps = {
  ...
  mwcPoolData: Array<MWCPoolBlockData>
}
...
export class MinerBlockRewards extends Component<MinerBlockRewardsProps, MinerBlockRewardsState> {
  ...
  fetchData = () => {
    const { fetchBlockRange, fetchNetworkData, fetchMWCPoolData, fetchMinerShareData } = this.props
    const { range } = this.state
    fetchBlockRange(null, range)
    fetchNetworkData()
    fetchMWCPoolData()
    ...
    if (recentBlocks.length) {
      for (let i = 1; i <= range && i <= recentBlocks.length - 1; i++) {
        ...
        if (block.height > latestBlockHeight - 1440 + 250 && block.height < latestBlockHeight - 5) {
          const blockRewardData = getMinerBlockRewardData(block.height, networkData, mwcPoolData, minerShareData)
          if (blockRewardData) {
            ...
            if (!isNaN(poolCredit) && !isNaN(minerCredit)) minerPortion = `${(minerCredit / poolCredit * 100 * RATIO_POOL_FEE).toFixed(3)}%`
            userReward = blockRewardData.userReward * RATIO_POOL_FEE
          }
        }
        rows.push(
          <tr key={block.height}>
            <td style={{ textAlign: 'left' }}><a style={{ color: rowColor }} href={`https://explorer.mwc.mw/#b${block.height}`} rel='noopener noreferrer' target='_blank' >{block.height}</a></td>
