...
export const aggregateShareCreditsFromUserStats = (sharesRows): { c31: number} => {
...
export const aggregateShareCreditsFromPoolShares = (
  ...
  sharesRows.forEach((row) => {
    if (row.share_counts) {
      const parsedShareCounts: { C31?: any } = JSON.parse(row.share_counts)
...
export const sanitizeHeight = async (req, res, next) => {
  ...
  if (req.params && req.params.height && (parseInt(req.params.height) < 1)) {
    ...
    index.cache.get(`router_/mwc/block`, async (err, item: string) => {
      ...
      if (item) {
        ...
        index.cache.set(`router_/mwc/block.height`, maxHeight, 'EX', 1)
