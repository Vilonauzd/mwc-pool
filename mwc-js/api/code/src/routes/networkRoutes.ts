...
networkRouter.get('/stats/:height,:range/:fields?', sanitizeHeight, limitRange, async (req, res, next) => {
  try {
    ...
    const networkStatsRangeResults: DatabaseGrinStat[] = await knex
      .select(['mwc_stats.difficulty', 'gps.gps', 'gps.edge_bits', 'blocks.height', knex.raw('UNIX_TIMESTAMP(mwc_stats.timestamp) as timestamp'), 'blocks.secondary_scaling'])
      .from('mwc_stats')
      .join('gps', 'mwc_stats.height', '=', 'mwc_stats_id')
      .join('blocks', 'mwc_stats.height', '=', 'blocks.height')
      .where('mwc_stats.height', '>', min)
      .andWhere('mwc_stats.height', '<=', finalHeight)
