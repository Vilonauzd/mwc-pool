      ...
      if (getUserEstimatedRewardsCache) {
        ...
        try {
          ...
          cache.get(poolBlockShareWeightCacheKey, async (error, poolSharesCreditCache: string) => {
            ...
            const poolSharesCredit: {c31: number} = JSON.parse(poolSharesCreditCache)
            if (poolSharesCredit) {
              ...
              cache.get(userBlockShareWeightCacheKey, async (error, userSharesCreditCache: string) => {
                if (error) throw { statusCode: 500, message: 'Error getting cache' }
                const userSharesCredit: { c31: number } = JSON.parse(userSharesCreditCache)
                if (userSharesCredit) {
                ...
                const aggregatePoolSharesWeight = poolSharesWeights.c31
                const aggregateUserSharesWeight = userSharesWeights.c31
                ...
                const blockReward = Math.floor( parseFloat(process.env.BLOCK_REWARD) * 1000000000 * blockRewardRate) // sent in nanogrin
