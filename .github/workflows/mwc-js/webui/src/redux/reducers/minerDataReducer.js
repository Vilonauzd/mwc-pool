...
const rigGpsData = (state: any = { c31: [] }, action: any) => {
  switch (action.type) {
    case 'RIG_DATA':
...
type MinerPaymentTxSlateAction = { type: 'MINER_PAYMENT_TX_SLATE', data: string } | { type: 'MINER_PAYMENT_TX_SLATE_FAILURE', data: string } | { type: 'ACCOUNT', data: null }
...
const minerPaymentTxSlate = (state: string = '', action: MinerPaymentTxSlateAction) => {
  switch (action.type) {
    ...
    case 'MINER_PAYMENT_TX_SLATE_FAILURE':
      return {
        message: action.data.message,
        color: 'danger',
        success: false
      }
    case 'MINER_PAYMENT_TX_SLATE_CLEAR':
      return {
        data: action.data,
        success: false
      }
