...
import { fetchMWCPoolBlocksMined } from './mwcPoolDataActions.js'
...
export const getLatestBlock = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    const latestBlockUrl = `${API_URL_V2}mwc/block`
...
export const fetchNetworkRecentBlocks = (endBlock: number | null = null, range: number = 20) => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    if (latestBlockHeight === 0) return
    const url = `${API_URL_V2}mwc/blocks/${latestBlockHeight},${range}`
    ...
    // const blocksOrphaned = MWCPoolRecentBlocksData.filter((block) => block.state === 'orphan')
    // console.log('fetchMWCPoolRecentBlocks blocksOrphaned: ', blocksOrphaned)
    dispatch({
...
export const getMinedBlocksAlgos = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    const combinedMax = Math.max(...previousData.c31)
    if (combinedMax > previousMaxBlockHeight) previousMaxBlockHeight = combinedMax
    const blockDifference = latestBlockHeight - previousMaxBlockHeight
    const minedBlockAlgosUrl = `${API_URL_V2}mwc/blocks/${latestBlockHeight},${blockDifference}/height,edge_bits`
    ...
    dispatch(fetchMWCPoolBlocksMined())
  } catch (e) {
    console.log('error: ', e)
  }
}
