...
#
# Common Routines for mwc-Pool Services
#
...
def get_config():
    ...
    with rlock:
        if CONFIG == None:
            print("Init new CONFIG", os.path.isfile('config.ini'))
...
def get_db():
    ...
    with rlock:
        if DATABASE is None:
            ...
            try:
                mwc_POOL_ADMIN_USER = os.environ['MWC_POOL_ADMIN_USER']
                mwc_POOL_ADMIN_PASSWORD = os.environ['MWC_POOL_ADMIN_PASSWORD']
                ...
                try:
                    ...
                    if user is None:
                        user = Users(
                                id = 1,
                                username = mwc_POOL_ADMIN_USER,
                                password = mwc_POOL_ADMIN_PASSWORD,
                            )
