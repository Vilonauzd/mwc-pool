...
#
# Routines for getting mwc Pool data
#
...
# Globals
NANOmwc = 1000000000 # 1 and 9 zeros
...
def init_pool_users(CONFIG, LOGGER, database):
    ...
    try:
        webui_user = os.environ["mwc_POOL_WEBUI_USER"]
        webui_pass = os.environ["mwc_POOL_WEBUI_PASSWORD"]
    except KeyError:
...
def get_block_reward_nanomwc():
    BLOCK_REWARD_NANOmwc = get_block_reward() * NANOmwc
    return BLOCK_REWARD_NANOmwc
...
def get_reward_by_block(height):
    ...
    if theblock is None:
        return 0
    return get_block_reward_nanomwc() + theblock.fee
...
# Calculate expected mwc reward per day for 'gps' at 'height' using 'range' avg data
def estimated_daily_reward(gps=1, height=0, range=60):
    if height == 0:
        height=mwc.blocking_get_current_height()
    # Get Avg Network GPS
    network_gps_avgs = mwcstats.avg_network_gps(height, range)
    # Get POW Ratios
    C29Ratio = mwc.secondary_pow_ratio(height)
    PrimaryRatio = 100 - C29Ratio
    # Calculate Expected mwc Reward per Day
    estReward = {}
    for powsize, gpsavg in network_gps_avgs.items():
        if powsize == mwc.SECONDARY_SIZE:
            estReward[powsize] = float(gps) / network_gps_avgs[powsize] * C29Ratio/100.0 * mwc.DAY_HEIGHT * get_block_reward()
        else:
            estReward[powsize] = float(gps) / network_gps_avgs[powsize] * PrimaryRatio/100.0 * mwc.DAY_HEIGHT * get_block_reward()
    return estReward
...
def calculate_block_payout_map(height, window_size, pool_fee, logger, estimate=False):
    block_payout_map = {}
    # Get the mwcpool admin user ID for pool fee
...
#        }
    ...
    for user_id, worker_shares_count in shares_count_map.items():
        ...
        if user_id in block_payout_map.keys():
            ...
            block_payout_map[user_id] = worker_payment
    logger.warn("Total mwc Paid Out this block: {} + the_pools_fee: {} ".format(total_payments_this_block, the_pools_fee))
