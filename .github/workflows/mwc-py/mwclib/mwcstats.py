...
#
# Routines for working with mwc_stats records
#
...
def avg_network_gps(height=0, range=60):
    ...
    if range <= 0:
        range = 1
    mwcstats = mwc_stats.get_by_height(height, range)
    gpslists = [stat.gps for stat in mwcstats]
...
def estimate_all_gps(window):
    ...
    # Based on jaspervdm code - https://github.com/jaspervdm/mwc_mining_sim
    ...
    # ratios
    srr = mwc.secondary_pow_ratio(height)/100.0
    prr = 1.0 - srr
    # Calculate the GPS
    all_gps = []
    for edge_bits in counts.keys():
        ...
        if edge_bits == SECONDARY_SIZE:
            ...
            weight = mwc.graph_weight(edge_bits)
        # Caclulate GPS
...
# Calculate the mwc stats for the specified height
# Return a mwc_stats object
# Raises AssertionError
def calculate(height, avg_range=DIFFICULTY_ADJUST_WINDOW):
    # Get the most recent blocks from which to generate the stats
    recent_blocks = []
    previous_stats_record = mwc_stats.get_by_height(height-1)
