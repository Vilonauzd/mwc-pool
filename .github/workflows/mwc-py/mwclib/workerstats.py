...
#    pp.pprint(window)
    first_mwc_block = Blocks.get_by_height(first_height)
    last_mwc_block = Blocks.get_by_height(last_height)
    assert first_mwc_block is not None, "Missing mwc block at height: {}".format(first_height)
    assert last_mwc_block is not None, "Missing mwc block at height: {}".format(last_height)
...
def calculate(height, window_size):
    database = lib.get_db()
    mwc_block = Blocks.get_by_height(height)
    assert mwc_block is not None, "Missing mwc block at height: {}".format(height)
...
def recalculate(start_height, avg_range):
    database = lib.get_db()
    height = start_height
    while height <= mwc.blocking_get_current_height():
