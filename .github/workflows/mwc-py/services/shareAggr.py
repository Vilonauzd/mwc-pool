...
from mwcbase.model.worker_stats import Worker_stats # XXX TODO: mark worker_stats record dirty if that users worker_shares record is updated
...
def addPoolBlock(logger, timestamp, height, hash, found_by, serverid):
    ...
    try:
        ...
        actual_difficulty = mwc.difficulty(this_block.hash, this_block.edge_bits, this_block.secondary_scaling)
        net_difficulty = mwc.get_network_difficulty(height)
...
def poolblock_handler(ch, method, properties, body):
    ...
    if content["type"] == "poolblocks":
        ...
        s_height = int(content["height"])
        s_hash = content["hash"]
        s_worker =  1 # mwc log does not know, credit Pool Admin
        ...
        try:
            ...
            try:
                ...
                if share_height is None:
                    share_height = mwc.blocking_get_current_height()
