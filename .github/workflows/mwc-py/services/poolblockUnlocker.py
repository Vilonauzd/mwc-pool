...
def main():
    ...
    LOGGER.warning("using locktime: {}, expiretime: {}".format(block_locktime, block_expiretime))

    latest = mwc.blocking_get_current_height()
    LOGGER.warning("Latest: {}".format(latest))
    ...
    for pb in new_poolblocks:
        if pb.height < (latest - block_expiretime):
            ...
            pb.state = "expired"
            continue
        response = mwc.get_block_by_height(pb.height)
        ...
        if int(response["header"]["nonce"]) != int(pb.nonce):
            LOGGER.warning("Processed orphan pool block at height: {}".format(pb.height))
            pb.state = "orphan"
            continue
#        # Check that we have a coinbase output in the wallet for this block
#        if pb.height not in wallet_output_heights:
#            LOGGER.warning("Wallet has no output for pool block at height: {}".format(pb.height))
...
#            continue
        ...
        if pb.height < (latest - block_locktime):
            ...
            LOGGER.warning("Unlocking pool block at height: {}".format(pb.height))
            pb.state = "unlocked"
        sys.stdout.flush()
