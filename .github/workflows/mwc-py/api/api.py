...
from mwcbase.model.pool_blocks import Pool_blocks
from mwcbase.model.worker_stats import Worker_stats
from mwcbase.model.worker_shares import Worker_shares
from mwcbase.model.gps import Gps
from mwcbase.model.shares import Shares
from mwcbase.model.pool_utxo import Pool_utxo
from mwcbase.model.pool_payment import Pool_payment
from mwcbase.model.users import Users

from mwclib import lib
from mwclib import mwc
from mwclib import pool
...
# From K8s secret
admin_user = os.environ["MWC_POOL_ADMIN_USER"]
admin_pass = os.environ["MWC_POOL_ADMIN_PASSWORD"]
...
mwc_blocks_range_limit = 501
mwc_stats_range_limit = 240
...
def verify_password(username_or_token, password=None):
    ...
    token = username_or_token if ':random' not in username_or_token else username_or_token + ':' +password.replace(':random', '')
    LOGGER.warn("Will Verify User token: {}".format(token))
    user_rec = Users.verify_auth_token(app.config['SECRET_KEY'], token)
    if user_rec is None:
        # try to authenticate with username/password
        debug and LOGGER.warn("Will Verify User use rec: {}, {}".format(username_or_token, password))
        user_rec = Users.get(username_or_token, password)
    if user_rec is None:
        return False
    LOGGER.warn("Will Verify User token good: {}".format(token))
    g.user = user_rec
...
class PoolAPI_users(Resource):
    def post(self):
        ...
        if exists:
            debug and LOGGER.warn("Failed to add - conflict with existing user = {}".format(username))
            response = jsonify({ 'message': 'Conflict with existing account' })
            response.status_code = 409
            return response
        # Create the users record
        debug and LOGGER.warn("Added user = {} - ".format(username, password))
        user_rec = Users.create(username, password)
        if user_rec is None:
            debug and LOGGER.warn("Failed to add - unable to create a new user record")
        ...
        redis_key = redis_userid_key + user_rec.username
        r.set(redis_key, user_rec.id)
        debug and LOGGER.warn("Added user = {}".format(user_rec))
...
##
# Stats
class mwcAPI_stats(Resource):
    #decorators = [limiter.limit("50/minute")]
    @cache.cached(timeout=10)
    def get(self, height=None, range=None, fields=None):
        LOGGER = lib.get_logger(PROCESS)
        debug and LOGGER.warn("mwcAPI_stats get height:{} range:{} fields:{}".format(height, range, fields))
        # Enforce range limit
        if range is not None:
            range = min(range, mwc_stats_range_limit)
        fields = lib.fields_to_list(fields)
        if height is None or height == 0:
            stats = mwc_stats.get_latest(range)
        else:
            stats = mwc_stats.get_by_height(height, range)
        #pp.pprint(stats)
...
api.add_resource(mwcAPI_stats,
        '/mwc/stat',
        '/mwc/stat/<string:fields>',
        '/mwc/stat/<int:height>',
        '/mwc/stat/<int:height>/<string:fields>',
        '/mwc/stats/<int:height>,<int:range>',
        '/mwc/stats/<int:height>,<int:range>/<string:fields>',
)

##
# Blocks
class mwcAPI_blocks(Resource):
    #decorators = [limiter.limit("50/minute")]
    @cache.cached(timeout=10)
    def get(self, height=None, range=None, fields=None):
        LOGGER = lib.get_logger(PROCESS)
        debug and LOGGER.warn("mwcAPI_blocks get height:{} range:{} fields:{}".format(height, range, fields))
        # Enforce range limit
        if range is not None:
            range = min(range, mwc_blocks_range_limit)
        fields = lib.fields_to_list(fields)
        if height is None or height == 0:
...
api.add_resource(mwcAPI_blocks,
        '/mwc/block',
        '/mwc/block/<string:fields>',
        '/mwc/block/<int:height>',
        '/mwc/block/<int:height>/<string:fields>',
        '/mwc/blocks/<int:height>,<int:range>',
        '/mwc/blocks/<int:height>,<int:range>/<string:fields>')
...
class WorkerAPI_utxo(Resource):
    ...
    def post(self, id, field, value):
        ...
        allowed_fields = ["address", "method", "min_payout", "enable_auto_payout"]
        ...
        elif field == "enable_auto_payout":
            st = Pool_utxo.update_enable_auto_payout(id, value)
        if st == False:
...
class EstimateApi_payment(Resource):
    ...
    def get(self, id, height=None, range=None):
        LOGGER = lib.get_logger(PROCESS)
        LOGGER.warn("YOLO {} - {}".format(id, g.user.id))
        if id != g.user.id:
            response = jsonify({ 'message': 'Not authorized to access data for other users' })
            response.status_code = 403
            return response
...
class PoolAPI_paymentrequest(Resource):
    ...
    def post(self, id, function, address=None):
        ...
        LOGGER = lib.get_logger(PROCESS)
        debug = True
        debug and LOGGER.warn("PoolAPI_payment request POST: {} - {}".format(id, function))
        # AUTH FILTER
        if id != g.user.id:
            response = jsonify({ 'message': 'Not authorized to access data for other users' })
            response.status_code = 403
        ...
        #if id != 573: # foo16 on bitmwc pool for testing
        ...
        if function == "get_tx_slate":
            ...
            get_tx_slate_url = payment_req_url + "/pool/payment/get_tx_slate/"+str(id)
            ...
            # Online Wallet-To-Wallet
            debug and LOGGER.warn("Send TOR transaction: {}".format(id))
            LOGGER.warn("request.args: {}".format(request.args))
            if address is None:
                LOGGER.warn("TOR payment request missing address")
                response = jsonify({ 'message': 'Error, must specify a wallet address:port' })
                response.status_code = 400
                return response
            debug and LOGGER.warn("Initiate TOR payment: {} - {}  - {}".format(id, address, request.args))
            LOGGER.warn("Args in json: {}".format(json.dumps(request.args)))
            tor_payment_url = payment_req_url + "/pool/payment/tor/{}/{}".format(id, address)
            r = requests.post(
                    url=tor_payment_url,
                    data=json.dumps(request.args),
                    headers={'content-type': 'application/json'},
            ...
            if r.status_code != 200:
                LOGGER.warn("Failed to complete TOR payment: {} - {}".format(r.status_code, r.reason))
                response = jsonify({ 'message': 'Failed to complete TOR payment: {}'.format(r.json()["message"]) })
                response.status_code = r.status_code
                return response
            debug and LOGGER.warn("tor payment result: {} - {}".format(r.status_code, r.text))
            return "ok"
        elif function == "mwcmqs":
            # XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            #
            #if id != 573: # foo16 on bitmwc pool for testing
            #    LOGGER.warn("HTTP payment requests are disabled")
            ...
            ##
            # Online Wallet-To-Wallet
            debug and LOGGER.warn("Send MWCMQS transaction: {}".format(id))
            LOGGER.warn("request.args: {}".format(request.args))
            if address is None:
                LOGGER.warn("MWCMQS payment request missing address")
                response = jsonify({ 'message': 'Error, must specify a wallet address:port' })
                response.status_code = 400
                return response
            debug and LOGGER.warn("Initiate MWCMQS payment: {} - {}  - {}".format(id, address, request.args))
            LOGGER.warn("Args in json: {}".format(json.dumps(request.args)))
            mwcmqs_payment_url = payment_req_url + "/pool/payment/mwcmqs/{}/{}".format(id, address)
            r = requests.post(
                    url=mwcmqs_payment_url,
                    data=json.dumps(request.args),
                    headers={'content-type': 'application/json'},
                    auth=(admin_user, admin_pass)
                )
            if r.status_code != 200:
                LOGGER.warn("Failed to complete MWCMQS payment: {} - {}".format(r.status_code, r.reason))
                response = jsonify({ 'message': 'Failed to complete MWCMQS payment: {}'.format(r.json()["message"]) })
                response.status_code = r.status_code
                return response
            debug and LOGGER.warn("mwcmqs payment result: {} - {}".format(r.status_code, r.text))
            return "ok"
