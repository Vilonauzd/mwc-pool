...
const CONFIG_FILE_NAME: &'static str = "mwc-pool.toml";

#[derive(Debug, Deserialize, Clone)]
pub struct Config {
    pub mwc_pool: PoolConfig,
    pub mwc_node: NodeConfig,
...
pub fn read_config() -> Config {
    ...
    match env::var("MWC_ADDRESS") {
        Ok(address) => {
            config.mwc_node.address = address;
    ...
    match env::var("POOL") {
        Ok(pool) => {
            config.mwc_pool.pool = pool.trim().to_string();
            println!("env pool: {:?}", config.mwc_pool);
        }
        Err(e) => {}
    }
