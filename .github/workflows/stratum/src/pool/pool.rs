...
use mwc_core::pow::Proof;
use mwc_core::core::BlockHeader;
use mwc_core::global::{ChainTypes, init_global_chain_type};
use mwc_core::ser::{deserialize, ser_vec};
...
fn block_header(pre_pow: String,
    ...
    let mut pv = mwc_core::ser::ProtocolVersion(2);
    let mut header_bytes = from_hex(&pre_pow).unwrap();
    ...
    let header: BlockHeader = deserialize(&mut &header_bytes[..], pv, DeserializationMode::default())?;
    Ok(header)
}
...
fn pool_to_chaintype(pool: String) -> ChainTypes {
    ...
    match pool.as_str() {
        "mwcpool" => { return ChainTypes::Mainnet; },
        "mwcfloopool" => { return ChainTypes::Floonet; },
...
impl Pool {
    ...
    pub fn new(config: Config) -> Pool {
        Pool {
            ...
            chain_type: pool_to_chaintype(config.mwc_pool.pool.clone()),
        }
    }
    ...
    pub fn run(&mut self) {
        ...
        init_global_chain_type(self.chain_type.clone());
                ...
                Some(shares) => {
                    for mut share in shares {
                        ...
                        if self.duplicates.contains_key(&share.pow) {
                            ...
                            let _ = worker.send_err("submit".to_string(), "Failed to validate solution".to_string(), -32502);
    ...
    debug!(
        ...
        e
                        ...
                        // B) Call into mwc_core::pow::verify_size()
                                let verify_result = mwc_core::pow::verify_size(&bh);
                                if ! verify_result.is_ok() {
                                        worker.status.rejected += 1;
                                        worker.add_shares(share.edge_bits, 0, 1, 0); // Accepted, Rejected, Stale
    debug!(
        "{} - Rejected due to failed mwc_core::pow::verify_size()",
        self.id,
    );
                                        worker.send_err("submit".to_string(), "Failed to validate solution".to_string(), -32502);
                                        continue; // Dont process this share anymore
