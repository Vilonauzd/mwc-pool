...
//!
//! An upstream MWC stratum server
//!
...
// ----------------------------------------
// Server Object - our connection to a stratum server - a MWC node
...
impl Server {
    ...
    pub fn new(cfg: Config) -> Server {
        Server {
            id: "MWCPool".to_string(),
            config: cfg,
            stream: None,
            protocol: StratumProtocol::new(),
            error: false,
            job: JobTemplate::new(),
            status: WorkerStatus::new("MWCPool".to_string()),
            buffer: String::with_capacity(4096),
        }
    }

    /// Connect to an upstream MWC Stratum Server
    ...
    pub fn connect(&mut self) -> Result<(), String> {
        ...
        let mwc_stratum_url = self.config.mwc_node.address.clone() + ":"
            + &self.config.mwc_node.stratum_port.to_string();
        warn!(
            "{} - Connecting to upstream stratum server at {}",
            self.id,
            mwc_stratum_url.to_string()
        );
        match TcpStream::connect(mwc_stratum_url.to_string()) {
            Ok(conn) => {
    ...
    /// Request status from the upstream MWC Stratum server - this is *pool* status (not individual
    /// worker status)
    // Currently unused
...
//    }
        ...
        match self.stream {
            Some(ref mut stream) => {
                let login_params = LoginParams {
                    login: self.config.mwc_node.login.clone().to_string(),
                    pass: self.config.mwc_node.password.clone().to_string(),
                    agent: self.id.clone(),
                };
    ...
    /// Request a new job template from the upstream MWC Stratum server
                        ...
                        match rpc_msg {
                            Some(message) => {
                                ...
                                if v["method"] == String::from("job") {
                                    ...
                                    // This is a RESPONSE from the upstream MWC Stratum Server
                                    ...
                                    if worker_id == self.id {
                                        trace!("RESPONSE for MWCPOOL: {}", worker_id);
